...
import { Calendar, Plus, DollarSign, Clock, AlertCircle, CheckCircle2, Repeat, CreditCard } from 'lucide-react'
...
export default function RecurringPage() {
    ...
    try {
      ...
      const mockData = [
        ...
        },
        {
          ...
          amount: 1499,
          frequency: 'yearly',
          nextPayment: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
        }
      ]
      
      const savedSubs = JSON.parse(localStorage.getItem('subscriptions') || JSON.stringify(mockData))
  ...
  const saveSubscription = async (subData) => {
    try {
      ...
      if (subData.id) {
        updatedSubs = subscriptions.map(s => s.id === subData.id ? subData : s)
      } else {
        updatedSubs = [...subscriptions, { ...subData, id: Date.now() }]
      }
