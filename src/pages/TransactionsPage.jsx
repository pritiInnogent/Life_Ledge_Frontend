  ...
  return (
    <div className="p-4 lg:p-6 bg-gray-50 min-h-screen">
      <div className="mb-6 lg:mb-8">
        <h1 className="text-2xl lg:text-3xl font-bold text-gray-800 mb-2">Transactions</h1>
        <p className="text-sm lg:text-base text-gray-600">View and manage all your financial transactions</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
        <div className="p-4 lg:p-6 border-b border-gray-100">
          <div className="flex flex-col gap-4">
            <div>
              <h2 className="text-lg lg:text-xl font-semibold text-gray-800">Transaction History</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-2 lg:gap-3">
              <select
                ...
                className="px-3 lg:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm lg:text-base"
              >
            ...
            <>
              ...
              <div className="block lg:hidden space-y-3">
                {paginatedTransactions.map((txn) => (
                  <div key={txn.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    ...
                    <div className="font-medium text-gray-800 text-sm">{txn.merchant}</div>
                    ...
                    {txn.notes && (
                      <div className="text-xs text-gray-600 mt-2 truncate">{txn.notes}</div>
                    )}
                  </div>
                ))}
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-4 px-3 font-semibold text-gray-700 text-sm">Type</th>
                      <th className="text-left py-4 px-3 font-semibold text-gray-700 text-sm">Date</th>
                      <th className="text-left py-4 px-3 font-semibold text-gray-700 text-sm">Merchant</th>
                      <th className="text-left py-4 px-3 font-semibold text-gray-700 text-sm">Notes</th>
                      <th className="text-right py-4 px-3 font-semibold text-gray-700 text-sm">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedTransactions.map((txn) => (
                      <tr key={txn.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-4 px-3 text-gray-700 text-sm">
                          {new Date(txn.date).toLocaleDateString('en-IN')}
                        </td>
                        <td className="py-4 px-3">
                          <div className="font-medium text-gray-800 text-sm">{txn.merchant}</div>
                        ...
                        <td className="py-4 px-3 text-gray-600 max-w-xs truncate text-sm">
                          {txn.notes || '-'}
                        </td>
                        <td className={`py-4 px-3 text-right font-semibold text-sm ${
                          getAmountColor(txn.typeTransaction)
                        }`}>
                          ₹ {fmtAmount(txn.amount)}
                        </td>
                    ...
                    {[...Array(Math.min(3, totalPages))].map((_, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-2 lg:px-3 py-2 rounded-lg text-xs lg:text-sm font-medium ${
                            currentPage === page
